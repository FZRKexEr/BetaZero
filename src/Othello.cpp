...
vector<array<int, 2>> Othello::flip_once(vector<vector<int>> temp, int x, int y) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    for (int j = 1;; j++) {
      int nx = x + dx[i] * j;
      int ny = y + dy[i] * j;
      if (nx >= size || ny >= size || nx < 0 || ny < 0 || temp[nx][ny] == -1) {
        tque.clear();
        break;
      }
      if (temp[nx][ny] == o) break;
      if (temp[nx][ny] == (o ^ 1)) tque.push_back({nx, ny});
    }
...
long long Othello::try_play(int x, int y) {
  ...
  temp[x][y] = o;
  vector<array<int, 2>> res = flip_once(temp, x, y);
...
void Othello::play(int x, int y) {
  ...
  board[x][y] = o;
  vector<array<int, 2>> res = flip_once(board, x, y);
  for (auto &it: res) {
    board[it[0]][it[1]] = o;
